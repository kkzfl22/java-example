...
public class Proxy {
  ...
  public static Object newProxyInstance(Class infce, InvocationHandler h) throws Exception {
    ...
    String namePath =
        infce.getClassLoader().getResource("com/liujun/pattern/dynamicproxy/jdk/").getPath();

    String className = "$Proxy1";
    String fileName = namePath + "/" + className + ".java";
    // 生成java文件
    FileWriter fw = null;
    try {
      ...
      e.printStackTrace();
      System.out.println("Proxy newProxyInstance Exception" + e);
    } finally {
  ...
  private static String getClassMsg(Class infce) {
    ...
    String src =
        "package com.liujun.pattern.dynamicproxy.jdk;"
            ...
            + "    }"
            + rt
            + "    com.liujun.pattern.dynamicproxy.jdk.InvocationHandler h;"
            + rt
            + methodStr
            + "}";
